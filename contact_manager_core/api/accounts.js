...
*/
  ...
  this.CreateAccount = function CreateAccount(req,res){
    ...
    function StartCreateAccount(){
      ...
      var createAccountModel = new AccountModel(data, "CREATE");
      createAccountModel.ValidateModel(ValidationFailed, ValidationSuccess);
      function ValidationSuccess (){
        logger.log("AccountsAPI.ValidationSuccess", appModes.DEBUG);
        if (!createAccountModel.IsValid()){
          ValidationFailed();
          return;
        }
        var accountsDAO = new AccountsDataAccess();
        var account = createAccountModel.ToJSON().data;
        ...
        function CreateAccountFailed(err){
          ...
          response.account = createAccountModel.ToJSON();
          response.info = err;
...
/*
  ...
  "process: {validateApp, validateSession , read, validate(AccountModel), editAccount, returnResponse(success,error)}");
*/
  this.UpdateAccount = function UpdateAccount(req,res){
    ...
    function StartUpdateAccount(){
      ...
      var accountsDAO = new AccountsDataAccess();
      var data = req.body;
      var updateAccountModel = undefined;
      ...
      function GetAccountSuccess(dbAccount){
        ...
        logger.log("db: " + JSON.stringify(dbAccount, null, 2), appModes.DEBUG);
        logger.log("data: " + JSON.stringify(data, null, 2), appModes.DEBUG);
        updateAccountModel = new AccountModel(dbAccount, "UPDATE");
        updateAccountModel.ValidateModel(ValidationFailed, ValidationSuccess);
        function ValidationFailed(){
          logger.log("AccountsAPI.UpdateAccount.GetAccountSuccess.ValidationFailed", appModes.DEBUG);
          var response = new Object();
          response.account = updateAccountModel.ToJSON();
          res.status(400).send(response);
        }
        function ValidationSuccess(account){
          ...
          var account = updateAccountModel.ToJSON().data;
  ...
  this.GetAccount = function GetAccount(req,res){
    logger.log("AccountsAPI.GetAccount", appModes.DEBUG);
    var authenticationAPI = new AuthenticationAPI();
    authenticationAPI.AuthenticateApplication(req, res, authenticationAPI.AuthenticateFailed, AuthenticateApplicationSuccess)
    function AuthenticateApplicationSuccess(){
      logger.log("AccountsAPI.GetAccount.AuthenticateApplicationSuccess", appModes.DEBUG);
      authenticationAPI.AuthenticateAccount(req,res, authenticationAPI.AuthenticateFailed, StartUpdateAccount);
    }
    function StartUpdateAccount(){
      logger.log("AccountsAPI.GetAccount.StartUpdateAccount", appModes.DEBUG);
      var accountsDAO = new AccountsDataAccess();
      var data = req.body;
      var updateAccountModel = undefined;
      data._id = req.params.accountId;
      ...
      function GetAccountFailed(err){
          logger.log("AccountsAPI.GetAccount.GetAccountFailed", appModes.DEBUG);
          var response = new Object();
          response.account = data;
          response.info = err;
          res.status(400).send(response);
      ...
      function GetAccountSuccess(dbAccount){
        logger.log("AccountsAPI.GetAccount.GetAccountSuccess", appModes.DEBUG);
        var response = new Object();
        response.account = dbAccount;
        res.status(200).send(response); 
      }
    }
  };
